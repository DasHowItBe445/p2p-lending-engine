...
import { useApproveTokens, usePlaceBorrowOrder, useTokenAllowance } from '@/hooks/use-lending-contract'
...
export function BorrowerPanel() {
  ...
  useEffect(() => {
    if (isOrderSuccess) {
      toast.success('Borrow order placed successfully!', {
  ...
  const handlePlaceOrder = async () => {
    ...
    await placeBorrowOrder(amount, interestRate)
  }
    ...
    <Card className="border-border bg-card">
      <CardHeader className="pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-lg font-semibold text-card-foreground">Borrower Panel</CardTitle>
            <CardDescription className="text-muted-foreground">Borrow assets at competitive rates</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-col gap-3 pt-2">
          ...
          <Button
            className="w-full bg-chart-4 text-background hover:bg-chart-4/90"
